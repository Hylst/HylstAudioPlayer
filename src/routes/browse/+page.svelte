<script lang="ts">
  ...
  const dirCount = $derived(
    entries.filter((e) => e.kind === "directory").length,
  );
</script>
...
<div class="flex flex-col min-h-full bg-transparent">
  <!-- Header -->
  <header
    class="sticky top-0 z-30 flex items-center gap-3 px-4 pt-12 pb-4
    ...
    <div class="flex-1 min-w-0">
      ...
      {#if !loading && entries.length > 0}
        <p class="text-xs text-white/35">
          {dirCount} folders · {audioCount} audio files
        </p>
      {/if}
    </div>
  ...
  {#if stack.length > 1}
    <div class="flex items-center gap-1 px-4 py-2 overflow-x-auto no-scrollbar">
      ...
      {#each stack as crumb, i}
        <span
          class="material-symbols-rounded text-[14px] text-white/20 shrink-0"
          >chevron_right</span
        >
        {#if i === stack.length - 1}
          <span class="text-xs text-white/80 font-semibold truncate shrink-0"
            >{crumb.name}</span
          >
        {:else}
          <button
  ...
  <main class="flex-1 px-4 py-2 pb-6">
    {#if error}
      <div class="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <button onclick={loadRoot} class="text-xs text-indigo-400 underline"
          >Go to root</button
        >
      </div>
    {:else if loading}
      <div class="flex flex-col gap-2 pt-4">
        {#each Array(8) as _}
          ...
          >
            <div
              class="w-9 h-9 rounded-lg animate-pulse"
              style="background:rgba(255,255,255,0.07)"
            ></div>
            <div class="flex-1">
              <div
                class="h-3 rounded-full w-2/3 animate-pulse"
    ...
    {:else if entries.length === 0 && fsManager.rootHandles.length === 0}
      <div class="flex flex-col items-center gap-5 py-20 text-center">
        <span
          class="material-symbols-rounded text-6xl"
          style="color:rgba(255,255,255,0.15)">folder_off</span
        >
    ...
    {:else if entries.length === 0}
      <div class="flex flex-col items-center gap-4 py-20 text-center">
        <span
          ...
          style="color:rgba(255,255,255,0.15)">folder_open</span
        >
      ...
      <div class="flex flex-col gap-1">
        {#each entries as entry}
          {#if entry.kind === "directory"}
            <button
              onclick={() =>
                enterDir(entry.handle as FileSystemDirectoryHandle)}
            ...
            >
              ...
              <span
                class="flex-1 min-w-0 text-sm font-medium text-white/85 truncate"
                >{entry.name}</span
              >
              <span class="material-symbols-rounded text-[18px] text-white/20"
                >chevron_right</span
              >
            </button>
          {:else if entry.isAudio}
            <button
              ...
              style={player.currentTrack?.title ===
              entry.name.replace(/\.[^.]+$/, "")
            ...
            >
              ...
              >
                <span
                  class="material-symbols-rounded text-[20px] group-hover:text-indigo-300 transition-colors"
                  style="color:{colorFor(entry)}"
                >
                  {iconFor(entry)}
                </span>
              ...
              >
              <span
                class="material-symbols-rounded text-[16px] text-white/20 group-hover:text-white/50 transition-colors"
                >play_arrow</span
              >
            </button>
          {:else}
            ...
            <div
              class="flex items-center gap-3 px-3 py-2.5 rounded-xl opacity-40"
            >
              <span
                class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                style="background:rgba(255,255,255,0.04)"
              >
                <span class="material-symbols-rounded text-[18px] text-white/40"
                  >{iconFor(entry)}</span
                >
              </span>
              <span class="flex-1 min-w-0 text-sm text-white/50 truncate"
                >{entry.name}</span
              >
            </div>
          {/if}
