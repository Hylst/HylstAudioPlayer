...
<div class="flex flex-col min-h-dvh bg-hap-bg relative">
	...
	<main
		...
		class:pb-40={!player.currentTrack}
		class:pb-48={!!player.currentTrack}
	>
		{@render children()}
	</main>
  ...
  -->
	{#if !player.currentTrack}
		<nav
			class="fixed bottom-0 left-0 right-0 z-40 glass-panel-strong border-t border-hap-border"
			aria-label="Main navigation"
		>
			<div
				class="flex items-center justify-around max-w-md mx-auto h-20 px-4"
			>
				{#each navItems as item}
					...
					>
						<span
							class="material-symbols-outlined text-[26px] text-hap-text-muted group-hover:text-hap-primary transition-colors"
						>
							{item.icon}
						</span>
						<span
